...
type AddExpenseInput = {
  amount: number;
  category: string;       // keep required here because the select is required
  text: string;
  date: string;           // ISO date string: YYYY-MM-DD from the <input type="date" />
};
...
const AddRecord = () => {
  ...
  const [date, setDate] = useState<string>(''); // <-- added
  ...
  const clientAction = async () => {
    ...
    const { error } = await addExpenseRecord(payload);
          ...
          <div className="flex items-center gap-2">
            <div
              className={`w-6 h-6 rounded-full flex items-center justify-center ${
                alertType === 'success' ? 'bg-green-100 dark:bg-green-800' : 'bg-red-100 dark:bg-red-800'
              }`}
            >
